...
async def send_password_changed_notification(email: str):
    ...
    smtp_client = SMTPClient(
        host=SMTP_HOST, port=SMTP_PORT, username=SENDER_EMAIL, password=SENDER_PASSWORD
    )
...
class UserCreateMixin:
    ...
    async def register_user(user_form: UserRegisterForm):
        ...
        if not user_form.name or len(user_form.name.strip()) < 2:
            raise HTTPException(
                status_code=400, detail="Name need to have minimum 2 characters"
            )

        if not user_form.password or len(user_form.password) < 8:
            raise HTTPException(
                status_code=400, detail="Password need to have minimum 8 characters"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Ошибка при отправке кода: {str(e)}"
            )
    ...
    async def verify_email_code(request: Request):
        ...
        if not expected_code:
            raise HTTPException(
                status_code=400, detail="Код верификации не найден или истек"
            )
        ...
        jwt_token = await create_jwt_token(
            user.id, user.email, user.language, user.role
        )
